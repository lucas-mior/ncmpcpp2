...
static int32 cstrlen32(char *string);
...
test_mpd_reload_and_current_items(void) {
    ...
    assert(STREQUAL(mpd_fixture.content_path,
                            cstrlen32(mpd_fixture.content_path),
                            LIT_ARGS("Fetched")));
...
test_mouse_right_click_actions(void) {
    ...
    assert(STREQUAL(mpd_fixture.added_song_uri,
                            cstrlen32(mpd_fixture.added_song_uri),
                            LIT_ARGS("two.flac")));
...
test_locate_song_uses_legacy_playlist_order(void) {
    ...
    assert(STREQUAL(mpd_fixture.no_info_content_path,
                            cstrlen32(mpd_fixture.no_info_content_path),
                            LIT_ARGS("Target")));
    assert(STREQUAL(mpd_fixture.content_path,
                            cstrlen32(mpd_fixture.content_path),
                            LIT_ARGS("Target")));
...
test_playlist_commands(void) {
    ...
    assert(STREQUAL(mpd_fixture.command_target,
                            cstrlen32(mpd_fixture.command_target),
                            LIT_ARGS("Renamed")));
...
__wrap_nc_window_print_data(NcWindow *window, char *string,
    ...
    memcpy64(window_trace.printed + window_trace.printed_len,
           string, string_len);
...
__wrap_ncm_statusbar_print(int32 delay_seconds, char *message,
    ...
    if (message_len > 0) {
        memcpy64(mpd_fixture.status_message, message, message_len);
    }
...
__wrap_ncm_action_add_song_to_playlist(NcmSong *song, bool play,
    ...
    if (uri.len > 0) {
        memcpy64(mpd_fixture.added_song_uri, uri.data, uri.len);
    }

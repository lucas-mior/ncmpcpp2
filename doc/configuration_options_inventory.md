...
As of step 17, `src/configuration_options.def` is the authoritative production
definition for all effective configuration options. It generates their
...
handling move to the runtime application stage.  In step 14, parser-time
...
production X-macro list.  In step 16, the production configuration path invokes
the runtime stage before environment and command-line overrides, terminal title
capability moves into the title subsystem, and the two unused non-option fields
are removed. Every remaining `Configuration` field is generated by an option
entry or as an intrinsic companion of one. In step 17,
`configuration_options_pass.h` centralizes no-op/cleanup handling for X-macro
...
| `enable_window_title` | `yes` | `XX_BOOL` | `enable_window_title` | generated bool parser | Parse `yes`/`no` without consulting the environment. Runtime application configures title output separately; unsupported terminals do not rewrite this field. | trivial scalar |
...
- Configuration option parsing does not mutate `global_mpd`; MPD host, port,
  password, and timeout are applied by the production runtime stage before
  environment and command-line overrides.
- `enable_window_title` parsing is environment-independent.  Terminal capability
  is owned by the title subsystem and does not mutate the parsed configuration
  value.
- Runtime precedence is defaults/configuration file, then `MPD_HOST`/`MPD_PORT`,
  then command-line host/port overrides.

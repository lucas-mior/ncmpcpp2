...
1. Treat `src/screens/nc_lastfm.c`, `src/screens/nc_lastfm.h`, and `src/lastfm_service.c` as the existing C replacement for the C++ `Lastfm` screen and verify that `native_c_screens_init_all()` initializes `native_c_screen_lastfm`.
2. Replace every remaining `myLastfm`/`Lastfm` caller with `native_c_screen_lastfm` plus `native_lastfm_screen_queue_artist_info()`, starting with the artist-info action path in `src/actions.c`.
3. Register the native last.fm screen unconditionally through the native screen registry instead of creating the C++ screen from `actions_legacy.cpp`.
4. Wire the screen-switch path so `SHOW_ARTIST_INFO` queues the current artist, switches to `NCM_SCREEN_TYPE_LASTFM`, updates the title, and returns to the previous screen exactly like the legacy screen did.
5. Finish parity in `nc_lastfm.c`: render the same loading, empty, error, service-name, URL, similar-artist, and similar-tag sections that `lastfm.cpp` displayed.
6. Port the legacy scroll/mouse behavior by forwarding mouse events to the `NcScrollpad` instead of leaving the C callback as a no-op.
7. Verify the async job lifecycle: start the C job queue during native screen init, dispatch completed jobs from the app update loop, and stop/destroy it during shutdown.
8. Remove `screens/lastfm.h` dependencies from legacy/runtime headers once all call sites use the C API.
9. Delete the `#include "screens/lastfm.cpp"` line from `src/actions_legacy.cpp` and confirm no symbol from `lastfm.cpp` is referenced by any remaining object.
10. Delete `src/screens/lastfm.cpp` after adding/updating tests for duplicate artist fetches, failed fetches, and native screen registration.

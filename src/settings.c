...
apply_mpd_port(Configuration *config, char *value, int32 value_len,
    ...
    (void)config;
    status = ncm_parse_int32(value, value_len, &port, ncm_error);
    if (status < 0) {
        return status;
    }
    if (port > 65535) {
...
apply_visualizer_color(Configuration *config, char *value, int32 value_len,
                       NcmError *ncm_error) {
    NcmFormattedColorArray *array = &config->visualizer_colors;
    int32 pos;
    int32 status;
    bool added;
...
apply_user_interface(Configuration *config, char *value, int32 value_len,
    ...
    status = ncm_design_parse(value, value_len, &config->design);
    if (status < 0) {
        return settings_invalid_value(ncm_error, value, value_len);
    }
    return 0;
...
apply_header_window_color(Configuration *config, char *value, int32 value_len,
                          NcmError *ncm_error) {
    return settings_parse_color(value, value_len, &config->header_color,
                                ncm_error);
}
...
apply_main_window_color(Configuration *config, char *value, int32 value_len,
                        NcmError *ncm_error) {
    return settings_parse_color(value, value_len, &config->main_color,
                                ncm_error);
}
...
#define OPT(NAME, DEFAULT_VALUE, APPLY)              \
    {                                                            \
        .name = (char *)#NAME,                                    \
...
OPT(visualizer_in_stereo, "yes", apply_visualizer_in_stereo),
#if defined(HAVE_FFTW3_H)
OPT(visualizer_type, "spectrum", apply_visualizer_type),
#else
OPT(visualizer_type, "ellipse", apply_visualizer_type),
#endif
OPT(visualizer_look, "●▮", apply_visualizer_look),
OPT(visualizer_fps, "60", apply_visualizer_fps),
OPT(visualizer_autoscale, "no", apply_visualizer_autoscale),
OPT(visualizer_spectrum_smooth_look, "yes", apply_visualizer_spectrum_smooth_look),
OPT(visualizer_spectrum_smooth_look_legacy_chars, "yes", apply_visualizer_spectrum_smooth_look_legacy_chars),
OPT(visualizer_spectrum_dft_size, "2", apply_visualizer_spectrum_dft_size),
OPT(visualizer_spectrum_gain, "10", apply_visualizer_spectrum_gain),
OPT(visualizer_spectrum_hz_min, "20", apply_visualizer_spectrum_hz_min),
OPT(visualizer_spectrum_hz_max, "20000", apply_visualizer_spectrum_hz_max),
OPT(visualizer_spectrum_log_scale_x, "yes", apply_visualizer_spectrum_log_scale_x),
OPT(visualizer_spectrum_log_scale_y, "yes", apply_visualizer_spectrum_log_scale_y),
OPT(visualizer_color, "blue, cyan, green, yellow, magenta, red", apply_visualizer_color),
OPT(system_encoding, "", apply_system_encoding),
OPT(playlist_disable_highlight_delay, "5", apply_playlist_disable_highlight_delay),
OPT(message_delay_time, "5", apply_message_delay_time),
OPT(song_list_format, "{%a - }{%t}|{$8%f$9}$R{$3%l$9}", apply_song_list_format),
OPT(song_status_format, "{{%a{ \"%b\"{ (%y)}} - }{%t}}|{%f}", apply_song_status_format),
OPT(song_library_format, "{%n - }{%t}|{%f}", apply_song_library_format),
OPT(alternative_header_first_line_format, "$b$1$aqqu$/a$9 {%t}|{%f} $1$atqq$/a$9$/b", apply_header_first_line_format),
OPT(alternative_header_second_line_format, "{{$4$b%a$/b$9}{ - $7%b$9}{ ($4%y$9)}}|{%D}", apply_header_second_line_format),
OPT(current_item_prefix, "$(yellow)$r", apply_current_item_prefix),
OPT(current_item_suffix, "$/r$(end)", apply_current_item_suffix),
OPT(current_item_inactive_column_prefix, "$(white)$r", apply_current_item_inactive_column_prefix),
OPT(current_item_inactive_column_suffix, "$/r$(end)", apply_current_item_inactive_column_suffix),
OPT(now_playing_prefix, "$b", apply_now_playing_prefix),
OPT(now_playing_suffix, "$/b", apply_now_playing_suffix),
OPT(browser_playlist_prefix, "$2playlist$9 ", apply_browser_playlist_prefix),
OPT(selected_item_prefix, "$6", apply_selected_item_prefix),
OPT(selected_item_suffix, "$9", apply_selected_item_suffix),
OPT(modified_item_prefix, "$3>$9 ", apply_modified_item_prefix),
OPT(song_window_title_format, "{%a - }{%t}|{%f}", apply_song_window_title_format),
OPT(browser_sort_mode, "type", apply_browser_sort_mode),
OPT(browser_sort_format, "{%a - }{%t}|{%f} {%l}", apply_browser_sort_format),
OPT(song_columns_list_format, "(20)[]{a} (6f)[green]{NE} (50)[white]{t|f:Title} (20)[cyan]{b} (7f)[magenta]{l}", apply_song_columns_list_format),
OPT(execute_on_song_change, "", apply_execute_on_song_change),
OPT(execute_on_player_state_change, "", apply_execute_on_player_state_change),
OPT(playlist_show_mpd_host, "no", apply_playlist_show_mpd_host),
OPT(playlist_show_remaining_time, "no", apply_playlist_show_remaining_time),
OPT(playlist_shorten_total_times, "no", apply_playlist_shorten_total_times),
OPT(playlist_separate_albums, "no", apply_playlist_separate_albums),
OPT(playlist_display_mode, "columns", apply_playlist_display_mode),
OPT(browser_display_mode, "classic", apply_browser_display_mode),
OPT(search_engine_display_mode, "classic", apply_search_engine_display_mode),
OPT(playlist_editor_display_mode, "classic", apply_playlist_editor_display_mode),
OPT(discard_colors_if_item_is_selected, "yes", apply_discard_colors_if_item_is_selected),
OPT(show_duplicate_tags, "yes", apply_show_duplicate_tags),
OPT(incremental_seeking, "yes", apply_incremental_seeking),
OPT(seek_time, "1", apply_seek_time),
OPT(volume_change_step, "2", apply_volume_change_step),
OPT(autocenter_mode, "no", apply_autocenter_mode),
OPT(centered_cursor, "no", apply_centered_cursor),
OPT(progressbar_look, "=>", apply_progressbar_look),
OPT(default_place_to_search_in, "database", apply_default_place_to_search_in),
OPT(user_interface, "classic", apply_user_interface),
OPT(data_fetching_delay, "yes", apply_data_fetching_delay),
OPT(media_library_hide_album_dates, "no", apply_media_library_hide_album_dates),
OPT(media_library_primary_tag, "artist", apply_media_library_primary_tag),
OPT(media_library_albums_split_by_date, "yes", apply_media_library_albums_split_by_date),
OPT(default_find_mode, "wrapped", apply_default_find_mode),
OPT(default_tag_editor_pattern, "%n - %t", apply_default_tag_editor_pattern),
OPT(header_visibility, "yes", apply_header_visibility),
OPT(statusbar_visibility, "yes", apply_statusbar_visibility),
OPT(connected_message_on_startup, "yes", apply_connected_message_on_startup),
OPT(titles_visibility, "yes", apply_titles_visibility),
OPT(header_text_scrolling, "yes", apply_header_text_scrolling),
OPT(cyclic_scrolling, "no", apply_cyclic_scrolling),
OPT(lyrics_fetchers, "azlyrics, genius, letras, musixmatch, tekstowo, vagalume, internet", apply_lyrics_fetchers),
OPT(follow_now_playing_lyrics, "no", apply_follow_now_playing_lyrics),
OPT(fetch_lyrics_for_current_song_in_background, "no", apply_fetch_lyrics_background),
OPT(store_lyrics_in_song_dir, "no", apply_store_lyrics_in_song_dir),
OPT(generate_win32_compatible_filenames, "yes", apply_generate_win32_compatible_filenames),
OPT(allow_for_physical_item_deletion, "no", apply_allow_for_physical_item_deletion),
OPT(lastfm_preferred_language, "en", apply_lastfm_preferred_language),
OPT(space_add_mode, "add_remove", apply_space_add_mode),
OPT(show_hidden_files_in_local_browser, "no", apply_show_hidden_files_in_local_browser),
OPT(screen_switcher_mode, "playlist, browser", apply_screen_switcher_mode),
OPT(startup_screen, "playlist", apply_startup_screen),
OPT(startup_slave_screen, "", apply_startup_slave_screen),
OPT(startup_slave_screen_focus, "no", apply_startup_slave_screen_focus),
OPT(locked_screen_width_part, "50", apply_locked_screen_width_part),
OPT(ask_for_locked_screen_width_part, "yes", apply_ask_for_locked_screen_width_part),
OPT(media_library_column_width_ratio_two, "1:1", apply_media_library_column_width_ratio_two),
OPT(media_library_column_width_ratio_three, "1:1:1", apply_media_library_column_width_ratio_three),
OPT(playlist_editor_column_width_ratio, "1:2", apply_playlist_editor_column_width_ratio),
OPT(jump_to_now_playing_song_at_start, "yes", apply_jump_to_now_playing_song_at_start),
OPT(ask_before_clearing_playlists, "yes", apply_ask_before_clearing_playlists),
OPT(ask_before_shuffling_playlists, "yes", apply_ask_before_shuffling_playlists),
OPT(display_volume_level, "yes", apply_display_volume_level),
OPT(display_bitrate, "no", apply_display_bitrate),
OPT(display_remaining_time, "no", apply_display_remaining_time),
OPT(regular_expressions, "extended", apply_regular_expressions),
OPT(ignore_leading_the, "no", apply_ignore_leading_the),
OPT(block_search_constraints_change_if_items_found, "yes", apply_block_search_constraints_change),
OPT(mouse_support, "yes", apply_mouse_support),
OPT(mouse_list_scroll_whole_page, "no", apply_mouse_list_scroll_whole_page),
OPT(lines_scrolled, "5", apply_lines_scrolled),
OPT(empty_tag_marker, "<empty>", apply_empty_tag_marker),
OPT(tags_separator, " | ", apply_tags_separator),
OPT(tag_editor_extended_numeration, "no", apply_tag_editor_extended_numeration),
OPT(media_library_sort_by_mtime, "no", apply_media_library_sort_by_mtime),
OPT(enable_window_title, "yes", apply_enable_window_title),
OPT(search_engine_default_search_mode, "1", apply_search_engine_default_search_mode),
OPT(external_editor, "nano", apply_external_editor),
OPT(use_console_editor, "yes", apply_use_console_editor),
OPT(colors_enabled, "yes", apply_colors_enabled),
OPT(empty_tag_color, "cyan", apply_empty_tag_color),
OPT(header_window_color, "default", apply_header_window_color),
OPT(volume_color, "default", apply_volume_color),
OPT(state_line_color, "default", apply_state_line_color),
OPT(state_flags_color, "default:b", apply_state_flags_color),
OPT(main_window_color, "yellow", apply_main_window_color),
OPT(color1, "white", apply_color1),
OPT(color2, "green", apply_color2),
OPT(progressbar_color, "black:b", apply_progressbar_color),
OPT(progressbar_elapsed_color, "green:b", apply_progressbar_elapsed_color),
OPT(statusbar_color, "default", apply_statusbar_color),
OPT(statusbar_time_color, "default:b", apply_statusbar_time_color),
OPT(player_state_color, "default:b", apply_player_state_color),
OPT(alternative_ui_separator_color, "black:b", apply_alternative_ui_separator_color),
OPT(window_border_color, "green", apply_window_border_color),

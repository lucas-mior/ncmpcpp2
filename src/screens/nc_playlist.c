...
playlist_switch_to(NcScreen *screen) {
    ...
    playlist = (PlaylistScreen *)screen;
...
playlist_resize(NcScreen *screen) {
    PlaylistScreen *playlist = (PlaylistScreen *)screen;
    NcScreenResizeParams params = app_controller_screen_resize_params(screen,
                                                                      true);

    playlist_screen_set_geometry(playlist, params.x_offset, params.width,
...
playlist_update(NcScreen *screen) {
    ...
    if ((delay == 0) || (global_timer_elapsed_seconds(playlist->highlight_timer)
            <= delay)) {
        return;
    }
...
playlist_draw_song(NcMenu *menu, NcWindow *window, void *item,
    ...
    if (Config.playlist_display_mode == NCM_DISPLAY_MODE_COLUMNS) {
        ...
        if (is_now_playing) {
            available_width -= utf8_width(Config.now_playing_suffix.data,
...
static void
playlist_activate_song(NcMenu *menu, void *item, int32 pos, void *user) {
...
playlist_screen_destroy(PlaylistScreen *screen) {
    ...
    playlist_screen_unregister(screen);
...
void
playlist_screen_set_geometry(PlaylistScreen *screen, int32 start_x, int32 width,
                             int32 main_start_y, int32 main_height) {
    if (screen == NULL) {
        return;
    }
...
void
playlist_screen_set_mouse_config(PlaylistScreen *screen, int32 lines_scrolled,
                                 bool scroll_whole_page) {
    if (screen == NULL) {
        return;
    }
...
playlist_apply_changed_songs(PlaylistScreen *screen,
                             NcmSongArray *songs, int32 playlist_length) {
    NcMenu *menu;
    bool was_filtered;
...
playlist_screen_reload_from_mpd(PlaylistScreen *screen, MpdClient *client,
    ...
    if (status == 0) {
        playlist_apply_changed_songs(screen, &songs, playlist_length);
    }
...
playlist_build_mutable_song(NcmSong *replacement,
                            NcmSong *current, MutableSong *edited) {
    StrView value;
    int32 mtime;
    int32 duration;
    ...
    for (uint32 i = 0; i < TAG_COUNT; i += 1) {
        ...
        for (int32 j = 0; ; j += 1) {
            if (!mutable_song_has_tag_view(edited, type, j, &value)) {
                break;
            }
            if (value.len <= 0) {
                continue;
            }
            ncm_song_add_tag(replacement, type, value.data, value.len);
        }
    }
...
playlist_screen_update_current_mutable_song(PlaylistScreen *screen,
    ...
    playlist_build_mutable_song(&replacement, current, song);
...
playlist_screen_now_playing_song(PlaylistScreen *screen,
    ...
    if (position < count) {
        ...
        if (ncm_song_position(item) == queue_position) {
            ncm_song_copy(song, item);
            return 0;
        }
    }
...
int32
playlist_screen_copy_sort_range(PlaylistScreen *screen, NcmSongArray *songs,
...
playlist_screen_apply_filter(PlaylistScreen *screen,
    ...
    nc_menu_set_display_callbacks(playlist_storage_menu(screen), callbacks);
...
static int32
playlist_search_menu(PlaylistScreen *screen, NcMenu *menu, NcmRegex *regex,
...
int32
playlist_screen_search(PlaylistScreen *screen, char *pattern, int32 pattern_len,
...
playlist_screen_set_selected_priority(PlaylistScreen *screen,
    ...
    playlist_screen_selected_songs(screen, &songs);

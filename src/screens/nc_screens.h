...
#define SCREEN_TYPE_BROWSER_ENTRY(XX)                                      \
    XX(SCREEN_TYPE_BROWSER, 1, browser, SCREEN_FLAG_STARTUP, browser)
#define SCREEN_TYPE_HELP_ENTRY(XX)                                         \
    XX(SCREEN_TYPE_HELP, 2, help, SCREEN_FLAG_STARTUP, help)
#define SCREEN_TYPE_LASTFM_ENTRY(XX)                                       \
    XX(SCREEN_TYPE_LASTFM, 3, last_fm, SCREEN_FLAG_STARTUP, lastfm)
#define SCREEN_TYPE_LYRICS_ENTRY(XX)                                       \
    XX(SCREEN_TYPE_LYRICS, 4, lyrics, SCREEN_FLAG_STARTUP, lyrics)
#define SCREEN_TYPE_MEDIA_LIBRARY_ENTRY(XX)                                \
    XX(SCREEN_TYPE_MEDIA_LIBRARY, 5, media_library, SCREEN_FLAG_STARTUP,   \
      media_library)
#define SCREEN_TYPE_OUTPUTS_ENTRY(XX)                                      \
    XX(SCREEN_TYPE_OUTPUTS, 6, outputs, SCREEN_FLAG_STARTUP, outputs)
#define SCREEN_TYPE_PLAYLIST_ENTRY(XX)                                     \
    XX(SCREEN_TYPE_PLAYLIST, 7, playlist, SCREEN_FLAG_STARTUP, playlist)
#define SCREEN_TYPE_PLAYLIST_EDITOR_ENTRY(XX)                              \
    XX(SCREEN_TYPE_PLAYLIST_EDITOR, 8, playlist_edit, SCREEN_FLAG_STARTUP, \
      playlist_edit)
#define SCREEN_TYPE_SEARCH_ENGINE_ENTRY(XX)                                \
    XX(SCREEN_TYPE_SEARCH_ENGINE, 9, search_engine, SCREEN_FLAG_STARTUP,   \
      search_engine)
#define SCREEN_TYPE_SELECTED_ITEMS_ADDER_ENTRY(XX)                         \
    XX(SCREEN_TYPE_SELECTED_ITEMS_ADDER, 10, selected_items_adder,         \
      SCREEN_FLAG_NONE, selected_items_adder)
#define SCREEN_TYPE_SERVER_INFO_ENTRY(XX)                                  \
    XX(SCREEN_TYPE_SERVER_INFO, 11, server_info, SCREEN_FLAG_NONE,         \
      server_info)
#define SCREEN_TYPE_SONG_INFO_ENTRY(XX)                                    \
    XX(SCREEN_TYPE_SONG_INFO, 12, song_info, SCREEN_FLAG_NONE, song_info)
#define SCREEN_TYPE_SORT_PLAYLIST_DIALOG_ENTRY(XX)                         \
    XX(SCREEN_TYPE_SORT_PLAYLIST_DIALOG, 13, sort_playlist_dialog,         \
      SCREEN_FLAG_NONE, sort_playlist_dialog)
#define SCREEN_TYPE_TAG_EDIT_ENTRY(XX)                                     \
    XX(SCREEN_TYPE_TAG_EDIT, 14, tag_edit, SCREEN_FLAG_STARTUP, tag_edit)
#define SCREEN_TYPE_TINY_TAG_EDIT_ENTRY(XX)                                \
    XX(SCREEN_TYPE_TINY_TAG_EDIT, 15, tiny_tag_edit, SCREEN_FLAG_NONE,     \
      tiny_tag_edit)
#define SCREEN_TYPE_VISUALIZER_ENTRY(XX)                                   \
    XX(SCREEN_TYPE_VISUALIZER, 16, visualizer, SCREEN_FLAG_STARTUP,        \
      visualizer)
...
#define SCREEN_NC_TYPE_ENUM_FIELD(                       \
    screen_type, nc_value, alias, flags, suffix          \
)                                                        \
    SCREEN_TYPE_NC_TYPE(screen_type) = nc_value,
...
typedef struct NcScreenOps {
    ...
    void (*action_change_finished)(NcScreen *);
...
void nc_screen_default_action_change_finished(NcScreen *);
...
void nc_screen_finish_action_change(NcScreen *);
...
StrView nc_screen_current_filter(NcScreen *);
...
StrView nc_screen_current_search_constraint(NcScreen *);
...
typedef struct MediaLibraryHooks {
    int32 (*list_tags)(void *, enum TagType, StrFlexList *,
                       NcmError *);
    int32 (*list_all_songs)(void *, NcmSongArray *, NcmError *);
    int32 (*search_songs)(void *, MediaLibrarySongQuery *, NcmSongArray *,
                          NcmError *);
...
media_library_screen_column_state(MediaLibraryScreen *,
                                  enum MediaLibraryColumn);
StrView media_library_screen_active_filter_constraint(MediaLibraryScreen *);
StrView media_library_screen_active_search_constraint(MediaLibraryScreen *);
...
int32 media_library_tags_from_strings(MediaLibraryTagArray *,
                                      StrFlexList *);
int32 media_library_tags_from_songs(MediaLibraryTagArray *, NcmSongArray *,
                                    enum TagType);
int32 media_library_albums_from_songs(MediaLibraryAlbumArray *,
                                      NcmSongArray *, enum MediaLibraryMode,
                                      enum TagType, char *, int32);
int32 media_library_copy_sorted_songs(NcmSongArray *, NcmSongArray *);
...
typedef struct PlaylistEditScreen {
    ...
    char *playlist_filter_constraint;
    char *content_filter_constraint;
    char *playlist_search_constraint;
    ...
    int32 window_timeout_ms;
    int32 active_column;
    int32 playlist_filter_constraint_len;
    int32 content_filter_constraint_len;
    int32 playlist_search_constraint_len;
    int32 content_search_constraint_len;
    int32 displayed_playlist_path_len;
    int32 observed_playlist_path_len;
...
int32 playlist_edit_screen_reload_playlists_from_mpd(PlaylistEditScreen *,
                                                     MpdClient *,
                                                     NcmError *);
int32 playlist_edit_screen_load_content(PlaylistEditScreen *, NcmSongArray *);
...
#define SEARCH_ENGINE_FIRST_SEPARATOR_ROW \
    ((int32)SEARCH_ENGINE_CONSTRAINT_COUNT)
...
#define SEARCH_ENGINE_SECOND_SEPARATOR_ROW (SEARCH_ENGINE_SEARCH_MODE_ROW + 1)
#define SEARCH_ENGINE_SEARCH_BUTTON_ROW (SEARCH_ENGINE_SECOND_SEPARATOR_ROW + 1)
#define SEARCH_ENGINE_RESET_BUTTON_ROW (SEARCH_ENGINE_SEARCH_BUTTON_ROW + 1)
#define SEARCH_ENGINE_RESULT_SEPARATOR_ROW (SEARCH_ENGINE_RESET_BUTTON_ROW + 1)
#define SEARCH_ENGINE_RESULT_SUMMARY_ROW \
    (SEARCH_ENGINE_RESULT_SEPARATOR_ROW + 1)
#define SEARCH_ENGINE_RESULT_END_SEPARATOR_ROW \
    (SEARCH_ENGINE_RESULT_SUMMARY_ROW + 1)
#define SEARCH_ENGINE_STATIC_ROW_COUNT \
    (SEARCH_ENGINE_RESULT_END_SEPARATOR_ROW + 1)
...
typedef struct SearchEngineScreen {
    ...
    int32 lines_scrolled;
    int32 result_len;
    int32 constraint_lens[SEARCH_ENGINE_CONSTRAINT_COUNT];
    int32 filter_constraint_len;
    int32 search_constraint_len;
...
typedef struct TagEditScreen {
    ...
    char *current_dir;
    char *observed_dir;
    char *highlighted_dir;
    char *directory_filter_constraint;
    char *tag_filter_constraint;
    ...
    int32 window_timeout_ms;
    int32 current_dir_len;
    int32 observed_dir_len;
    int32 highlighted_dir_len;
    int32 directory_filter_constraint_len;
    int32 tag_filter_constraint_len;
    int32 directory_search_constraint_len;
    int32 tag_search_constraint_len;
    int32 pattern_len;
...
int32 tag_edit_screen_current_dir(TagEditScreen *, StrView *);
int32 tag_edit_screen_current_directory_path(TagEditScreen *, StrView *);
...
typedef struct BrowserScreen {
    ...
    int32 last_highlighted_directory_len;
    int32 filter_constraint_len;
    int32 search_constraint_len;
...
StrView browser_screen_current_directory(BrowserScreen *);
...
bool browser_screen_has_current_directory_path(BrowserScreen *,
                                               StrView *);
bool browser_screen_has_current_playlist_path(BrowserScreen *, StrView *);

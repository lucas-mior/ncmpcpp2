...
static bool
browser_path_is_parent_directory(char *directory, int32 directory_len) {
    if (directory_len <= 0) {
        return false;
    }
...
static bool
browser_string_views_matches(StrView left, StrView right) {
...
browser_reload_from_local(BrowserScreen *screen, NcmError *ncm_error) {
    ...
    if (screen->current_directory.len <= 0) {
        str_set(&screen->current_directory, STRLIT("~"));
        status = ncm_path_expand_home(&screen->current_directory, ncm_error);
        if (status < 0) {
            return status;
        }
    } else {
        StrView current = browser_screen_current_directory(screen);
    ...
    if ((status == 0) && ((Config.browser_sort_mode == NCM_SORT_MODE_NONE)
    ...
    if (browser->update_requested) {
        ...
        if (browser_screen_is_local(browser)) {
            ...
            if (browser->current_directory.len <= 0) {
                ...
                StrView current = browser_screen_current_directory(browser);
            ...
            while (status >= 0) {
                ...
                StrView dir = browser_screen_current_directory(browser);
                char *path = dir.data;
...
browser_mouse_button_pressed(NcScreen *screen, MEVENT event) {
    ...
    if ((event.bstate & BUTTON5_PRESSED) != 0) {
        if (browser->mouse_list_scroll_whole_page) {
            nc_menu_scroll_selectable(menu, browser->main_height,
                                      NC_SCROLL_PAGE_DOWN);
        } else {
...
browser_item_matches(BrowserScreen *screen, NcmMpdItem *item,
    ...
    browser_sync_display_mode(screen);
    str_clear(&screen->item_text_buffer);
    switch (ncm_mpd_item_kind(item)) {
    case NCM_MPD_ITEM_DIRECTORY:
        ncm_directory_has_path_view(ncm_mpd_item_directory(item), &path);
    ...
    case NCM_MPD_ITEM_SONG:
        ...
        str_move(&screen->item_text_buffer, &rendered);
        str_free(&rendered);
        break;
    case NCM_MPD_ITEM_PLAYLIST:
...
void
browser_screen_init(BrowserScreen *screen, int32 start_x, int32 width,
...
browser_screen_destroy(BrowserScreen *screen) {
    ...
    str_free(&screen->scratch_buffer);
    str_free(&screen->item_text_buffer);
    ...
    str_free(&screen->column_title_text);
    str_free(&screen->title_text);
    stupid_string_free(&screen->last_highlighted_directory,
                       &screen->last_highlighted_directory_len);
    str_free(&screen->current_directory);
...
void
browser_screen_set_mouse_config(BrowserScreen *screen, int32 lines_scrolled,
...
void
browser_screen_add_item_move(BrowserScreen *screen, NcmMpdItem *item) {
    NcmMpdItem copy;
...
static int32
browser_compare_views(StrView left, StrView right) {
...
browser_screen_set_current_directory(BrowserScreen *screen,
    ...
    str_set(&screen->current_directory, directory, directory_len);
    return 0;
}
...
browser_screen_update_column_title(BrowserScreen *screen) {
    ...
    str_clear(&screen->column_title_text);
    if ((screen->active_display_mode != NCM_DISPLAY_MODE_COLUMNS)
...
browser_screen_fetch_supported_extensions(BrowserScreen *screen,
    ...
    for (int32 i = 0; i < strflex_list_len(&strings);
        ...
        if ((string->len <= 0) || (string->data[0] != '.')) {
            str_set(&buffer, STRLIT("."));
            STR_APPEND(&buffer, string->data, string->len);
        } else {
            str_set(&buffer, string->data, string->len);
        }
...
void
browser_screen_set_local(BrowserScreen *screen, bool local_browser) {
    if (screen == NULL) {
        return;
    }
...
int32
browser_screen_current_song(BrowserScreen *screen, NcmSong *song) {
    NcmMpdItem *item;
...
int32
browser_screen_selected_songs(BrowserScreen *screen, NcmSongArray *songs) {
    NcMenu *menu;
    int32 status;
...
browser_real_path(BrowserScreen *screen, StrView path,
    ...
    str_clear(real_path);
    if (screen->local_browser) {
        str_set(real_path, path.data, path.len);
        return ncm_error_ok(ncm_error);
    }
...
static int32
browser_delete_path_recursive(char *path, int32 path_len, NcmError *ncm_error) {
    NcmFsDirectory directory = {0};
    NcmFsEntry entry = {0};
    NcmFsStat stat = {0};
    int32 status;
...
static int32
browser_current_directory_item_path(BrowserScreen *screen, StrView *path,
                                    NcmError *ncm_error) {
    NcmMpdItem *item;
...
static int32
browser_current_playlist_item_path(BrowserScreen *screen, StrView *path,
                                   NcmError *ncm_error) {
    NcmMpdItem *item;
...
browser_screen_rename_current_directory(BrowserScreen *screen,
    ...
    new_path_view = ncm_string_view(new_path, new_path_len);
    {
        ...
        str_free(&new_real_path);
        str_free(&old_real_path);
    }
    if (status < 0) {
        return status;
    }
...
browser_screen_locate_song(BrowserScreen *screen,
    ...
    if (local_browser) {
        ...
        if (directory.len <= 0) {
            ...
            str_set(&path, directory.data, directory.len);
        }
...
int32
browser_screen_search(BrowserScreen *screen, char *pattern, int32 pattern_len,

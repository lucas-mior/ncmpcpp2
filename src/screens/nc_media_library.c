...
static bool native_library_active_item_matches(
    NativeMediaLibraryScreen *screen, NcMenu *menu, int32 pos,
    NcmRegex *regex);
static bool native_library_search_position(NcMenu *menu, int32 pos,
                                           void *user);
...
native_media_library_screen_current_primary_tag_value(
    ...
    if (screen->mode == NATIVE_MEDIA_LIBRARY_MODE_THREE_COLUMNS) {
        if ((tag = native_media_library_screen_current_tag(screen)) == NULL) {
            return false;
        }
        *value = tag->tag;
    ...
    if ((album = native_media_library_screen_current_album(screen)) == NULL) {
        return false;
    }
    *value = album->tag;
...
native_media_library_screen_current_album_value(
    ...
    if (((row = native_media_library_screen_current_album(screen)) == NULL)
...
native_library_append_tag(NativeMediaLibraryTagArray *tags,
    ...
    if ((row = native_media_library_tag_array_append(tags)) == NULL) {
        return false;
    }
...
native_library_append_album(NativeMediaLibraryAlbumArray *albums,
    ...
    if ((item = native_media_library_album_array_append(albums)) == NULL) {
        return false;
    }
    row = &item->row;
    if (!native_library_set_owned_string(&row->tag, &row->tag_len,
...
static bool
native_library_search_position(NcMenu *menu, int32 pos,
                               void *user) {
    NativeMediaLibrarySearchContext *context;
...
static void
native_library_restore_highlight(NcMenu *menu, int32 highlight) {
    int32 count;
...
native_library_clear_column_filter(NativeMediaLibraryScreen *screen,
    ...
    state = native_media_library_screen_column_state(screen, column);
    if ((state == NULL)
        || ((menu = native_library_column_menu(screen, column)) == NULL)) {
        return;
    }
...
native_library_mouse_button_pressed(NcScreen *screen,
    ...
    if ((library = native_library_from_screen(screen)) == NULL) {
        return;
    }
...
native_library_tag_filter(NcMenu *menu, void *item, void *user) {
    ...
    return native_library_tag_matches(
        screen, item,
        &screen->column_state[
            NATIVE_MEDIA_LIBRARY_COLUMN_TAGS].filter_regex);
}
...
native_library_album_filter(NcMenu *menu, void *item, void *user) {
    ...
    return native_library_album_matches(
        screen, row,
        &screen->column_state[
            NATIVE_MEDIA_LIBRARY_COLUMN_ALBUMS].filter_regex);
}
...
native_library_column_has_visible_items(
    ...
    if ((menu = native_library_column_menu(screen, column)) == NULL) {
        return false;
    }

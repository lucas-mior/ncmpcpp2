...
tiny_editor_can_run_current(NcScreen *screen) {
    return tiny_editor_action_runnable((TinyTagEditScreen *)screen);
}
...
tiny_editor_run_row(TinyTagEditScreen *screen, int32 row) {
    ...
    if (row == TINY_TAG_EDIT_CANCEL_ROW) {
        return tiny_editor_finish(screen);
    }
    return -NCM_ERROR_UNAVAILABLE;
}
...
tiny_tag_edit_screen_open_song(TinyTagEditScreen *screen, NcmSong *song,
    ...
    if ((screen == NULL) || (song == NULL) || (path == NULL)
        ...
        return TINY_TAG_EDIT_OPEN_INVALID_ARGUMENT;
    }
    if (ncm_song_is_stream(song)) {
        return TINY_TAG_EDIT_OPEN_STREAM;
    }
    ...
    if (status < 0) {
        ...
        return TINY_TAG_EDIT_OPEN_UNREADABLE_FILE;
    }
    ...
    return TINY_TAG_EDIT_OPEN_SUCCESS;
}

...
static int32 lyrics_build_amalgama_direct_url(
...
static int32 lyrics_collect_direct_urls(NcmLyricsFetcherDef *fetcher,
...
static int32 lyrics_fetch_page(NcmLyricsFetcherDef *fetcher,
                               ...
                               bool *retry);
static int32 lyrics_fetch_search(NcmLyricsFetcherDef *fetcher,
                                 NcmLyricsResult *result, char *artist,
                                 int32 artist_len, char *title,
                                 int32 title_len);
static int32 lyrics_collect_search_urls(NcmLyricsFetcherDef *fetcher,
                                        StrBuilderArray *out, char *data,
                                        int32 data_len, char *artist,
                                        int32 artist_len, char *title,
                                        int32 title_len);
static int32 lyrics_fetch_internet(NcmLyricsFetcherDef *fetcher,
                                   NcmLyricsResult *result, char *artist,
                                   int32 artist_len, char *title,
                                   int32 title_len);
...
lyrics_build_amalgama_direct_url(
    ...
    status = lyrics_append_slug_profile(&artist_slug, artist_profile,
                                        artist, artist_len);
    if (status < 0) {
        goto cleanup;
    }
    status = lyrics_append_slug_profile(&title_slug, title_profile,
                                        title, title_len);
    if (status < 0) {
        goto cleanup;
    }

    SB_APPEND(url, "https://www.amalgama-lab.com/songs/");
...
lyrics_build_direct_url_profiles(
    ...
    case NCM_LYRICS_FETCHER_AMALGAMA:
        status = lyrics_build_amalgama_direct_url(
            url, artist, artist_len, title, title_len, artist_profile,
            title_profile);
        break;
    case NCM_LYRICS_FETCHER_AZLYRICS:
        SB_APPEND(url, "https://www.azlyrics.com/lyrics/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
        sb_append_byte(url, '/');
        if ((status = lyrics_append_slug_profile(url, title_profile,
                                                 title, title_len)) < 0) {
            break;
        }
        SB_APPEND(url, ".html");
    ...
    case NCM_LYRICS_FETCHER_GENIUS:
        SB_APPEND(url, "https://genius.com/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
    ...
    case NCM_LYRICS_FETCHER_LETRASMUS:
        SB_APPEND(url, "https://www.letras.mus.br/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
    ...
    case NCM_LYRICS_FETCHER_PAROLES:
        SB_APPEND(url, "https://www.paroles.net/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
        SB_APPEND(url, "/paroles-");
        if ((status = lyrics_append_slug_profile(url, title_profile,
                                                 title, title_len)) < 0) {
            break;
        }
    ...
    case NCM_LYRICS_FETCHER_MUSIXMATCH:
        SB_APPEND(url, "https://www.musixmatch.com/lyrics/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
    ...
    case NCM_LYRICS_FETCHER_TEKSTOWO:
        SB_APPEND(url, "https://www.tekstowo.pl/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
    ...
    case NCM_LYRICS_FETCHER_VAGALUME:
        SB_APPEND(url, "https://www.vagalume.com.br/");
        if ((status = lyrics_append_slug_profile(url, artist_profile,
                                                 artist, artist_len)) < 0) {
            break;
        }
...
lyrics_collect_direct_urls(NcmLyricsFetcherDef *fetcher,
    ...
    for (int32 i = 0; i < LENGTH(pairs); i += 1) {
        status = lyrics_build_direct_url_profiles(
            fetcher, &candidate, artist, artist_len, title, title_len,
            pairs[i].artist, pairs[i].title);
        if (status < 0) {
            break;
        }
        status = lyrics_append_url_if_new(urls, candidate.data,
                                          candidate.len);
        if (status < 0) {
            break;
        }
...
ncm_lyrics_fetcher_fetch(NcmLyricsFetcherDef *fetcher, NcmLyricsResult *result,
    ...
    if (!result->success && retry
        ...
        status = lyrics_fetch_search(fetcher, result, artist, artist_len,
                                     title, title_len);
    }
...
lyrics_slug_match_score(char *wanted, int32 wanted_len,
    ...
    if ((wanted_len > candid_len)
        && lyrics_starts_with_ignore_case(wanted, wanted_len,
                                          candid, candid_len)
...
static bool
lyrics_url_path_has_ascii_letter(char *url, int32 url_len) {
    int32 path_start;
    int32 path_end;
...
lyrics_search_candidate_score(NcmLyricsFetcherDef *fetcher, char *url,
    ...
    if ((lyrics_append_slug(&wanted_artist, fetcher->type, artist,
                            artist_len) < 0)
...
lyrics_collect_search_urls(NcmLyricsFetcherDef *fetcher, StrBuilderArray *out,
    ...
    while (pos < unescaped.len) {
        ...
        if ((lyrics_unwrap_search_url(&candidate,
            ...
            status = lyrics_insert_search_url(out, scores, candidate.data,
                                              candidate.len, score);
            if (status < 0) {
                break;
            }
...
static int32
lyrics_lacoccinelle_content_end(char *data, int32 data_len, int32 start) {
    int32 best;

    best = -1;
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Publié par"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Chanteurs :"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Albums :"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Vos commentaires"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Commentaires"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Téléchargement"), &best);
    return best;
}
...
lyrics_musica_content_start(char *data, int32 data_len) {
    ...
    marker = lyrics_find_ignore_case(data, data_len, STRLIT(">LETRA<"), 0);
    if (marker < 0) {
        return -1;
    }
...
lyrics_musica_content_end(char *data, int32 data_len, int32 start) {
    ...
    best = -1;
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Significado de la letra"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Puntuar "), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Imprimir letra"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Letra añadida"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("+ Letras de"), &best);
    return best;
}
...
lyrics_paroles_content_start(char *data, int32 data_len) {
    ...
    second_marker = lyrics_find_ignore_case(
        data, data_len, STRLIT("Paroles de la chanson"),
...
lyrics_paroles_content_end(char *data, int32 data_len, int32 start) {
    ...
    best = -1;
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Paroles.net dispose"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Sélection du moment"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Les plus grands succès"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("On aime"), &best);
    lyrics_update_first_match(data, data_len, start,
                              STRLIT("Top traduction"), &best);
    return best;
}
    ...
    *plain_text = false;
    ...
    case NCM_LYRICS_FETCHER_LETRASMUS:
        return lyrics_extract_divs(out, data, data_len,
                                   STRLIT("class=\"lyric-original\""),
                                   false);
    case NCM_LYRICS_FETCHER_LACOCCINELLE:
        return lyrics_extract_lacoccinelle(out, data, data_len);
    case NCM_LYRICS_FETCHER_MUSICA:
...
lyrics_fetch_search(NcmLyricsFetcherDef *fetcher, NcmLyricsResult *result,
    ...
    status = ncm_lyrics_fetcher_build_url(fetcher, &search_url, artist,
                                          artist_len, title, title_len);
    if (status < 0) {
        goto cleanup;
    }

    status = lyrics_curl_perform(&data, search_url.data, search_url.len,
                                 NULL, 0, true, 15);
    if (status < 0) {
        message = lyrics_status_error(status, &message_len);
        (void)ncm_lyrics_result_set(result, false, message, message_len);

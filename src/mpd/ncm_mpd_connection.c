...
ncm_mpd_connection_tag_type(enum TagType tag) {
    switch ((int32)tag) {
    TAG_DEFS(NCM_MPD_CONNECTION_TAG_TYPE_CASE)
    default:
...
static int32
ncm_mpd_connection_recv_song_array(MpdConnection *connection,
                                   NcmSongArray *songs) {
...
int32
ncm_mpd_connection_send_idle(MpdConnection *connection, uint32 events) {
...
ncm_mpd_connection_recv_idle(MpdConnection *connection, bool disable_timeout,
    ...
    events = 0;
    if (mpd_events & MPD_IDLE_DATABASE) {
        events |= NCM_MPD_IDLE_DATABASE;
    }
    if (mpd_events & MPD_IDLE_STORED_PLAYLIST) {
        events |= NCM_MPD_IDLE_STORED_PLAYLIST;
    }
    if (mpd_events & MPD_IDLE_PLAYLIST) {
        events |= NCM_MPD_IDLE_PLAYLIST;
    }
    if (mpd_events & MPD_IDLE_PLAYER) {
        events |= NCM_MPD_IDLE_PLAYER;
    }
    if (mpd_events & MPD_IDLE_MIXER) {
        events |= NCM_MPD_IDLE_MIXER;
    }
    if (mpd_events & MPD_IDLE_OUTPUT) {
        events |= NCM_MPD_IDLE_OUTPUT;
    }
    if (mpd_events & MPD_IDLE_UPDATE) {
        events |= NCM_MPD_IDLE_UPDATE;
    }
    if (mpd_events & MPD_IDLE_OPTIONS) {
...
int32
ncm_mpd_connection_get_queue(MpdConnection *connection, NcmSongArray *songs) {
    NCM_MPD_RETURN_IF_ERROR(ncm_mpd_connection_require_connected(connection));
    if (songs == NULL) {
        return -EINVAL;
    }
    ...
    return ncm_mpd_connection_recv_song_array(connection, songs);
}

int32
ncm_mpd_connection_get_queue_changes(MpdConnection *connection,
                                     int32 version, NcmSongArray *songs) {
    NCM_MPD_RETURN_IF_ERROR(ncm_mpd_connection_require_connected(connection));
    if (songs == NULL) {
        return -EINVAL;
    }

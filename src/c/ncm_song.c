#if !defined(NCM_SONG_C)
#define NCM_SONG_C

#include "cbase.h"

#include <mpd/client.h>

#include "c/ncm_c.h"

static bool
ncm_song_needs_numeric_zero(char *tag, int32 tag_len) {
    ASSERT((tag != NULL) && (tag_len >= 0));

    if ((tag_len == 1) && (tag[0] != '0')) {
        return true;
    }
    if ((tag_len > 3) && (tag[1] == '/')) {
        return true;
    }

    return false;
}

static int32
ncm_song_format_numeric_tag_prefix(char *buffer, int32 buffer_cap,
                                   char *tag, int32 tag_len,
                                   int32 copy_len) {
    int32 out;

    ASSERT((buffer != NULL) && (buffer_cap > 0));
    ASSERT((tag != NULL) && (tag_len >= 0));
    ASSERT((copy_len >= 0) && (copy_len <= tag_len));

    out = 0;
    if (ncm_song_needs_numeric_zero(tag, tag_len)) {
        if (out + 1 < buffer_cap) {
            buffer[out] = '0';
        }
        out += 1;
    }

    for (int32 i = 0; i < copy_len; i += 1) {
        if (out + 1 < buffer_cap) {
            buffer[out] = tag[i];
        }
        out += 1;
    }

    if (out < buffer_cap) {
        buffer[out] = '\0';
    } else {
        buffer[buffer_cap - 1] = '\0';
        out = buffer_cap - 1;
    }

    return out;
}

static void
ncm_song_tag_init(NcmSongTag *tag) {
    tag->value = NULL;
    tag->value_len = 0;
    tag->type = MPD_TAG_UNKNOWN;
    return;
}

static void
ncm_song_tag_destroy(NcmSongTag *tag) {
    ASSERT(tag != NULL);

    free2(tag->value, tag->value_len + 1);
    ncm_song_tag_init(tag);

    return;
}

static void
ncm_song_grow_tags(NcmSong *song) {
    int32 old_cap;
    int32 new_cap;

    if (song->tags_len < song->tags_cap) {
        return;
    }

    old_cap = song->tags_cap;
    if (old_cap <= 0) {
        new_cap = 8;
    } else {
        new_cap = old_cap*2;
    }

    song->tags = (NcmSongTag *)realloc2(song->tags, old_cap,
                                        new_cap,
                                        SIZEOF(*song->tags));
    for (int32 i = old_cap; i < new_cap; i += 1) {
        ncm_song_tag_init(&song->tags[i]);
    }
    song->tags_cap = new_cap;
    return;
}

static void
ncm_song_destroy_unchecked(NcmSong *song) {
    free2(song->uri, song->uri_len + 1);
    for (int32 i = 0; i < song->tags_len; i += 1) {
        ncm_song_tag_destroy(&song->tags[i]);
    }
    free2(song->tags, song->tags_cap*SIZEOF(*song->tags));

    *song = (NcmSong){0};
    return;
}

static void
ncm_song_set_uri_unchecked(NcmSong *song, char *uri, int32 uri_len) {
    char *copy;

    copy = (char *)malloc2(uri_len + 1);
    memcpy64(copy, uri, uri_len);
    copy[uri_len] = '\0';

    free2(song->uri, song->uri_len + 1);
    song->uri = copy;
    song->uri_len = uri_len;
    return;
}

static void
ncm_song_add_tag_unchecked(NcmSong *song, enum mpd_tag_type type,
                           char *value, int32 value_len) {
    NcmSongTag *tag;

    ncm_song_grow_tags(song);

    tag = &song->tags[song->tags_len];
    ncm_song_tag_destroy(tag);
    tag->value = (char *)malloc2(value_len + 1);
    tag->value_len = value_len;
    tag->type = type;
    memcpy64(tag->value, value, value_len);
    tag->value[value_len] = '\0';
    song->tags_len += 1;
    return;
}

void
ncm_song_destroy(NcmSong *song) {
    if (song == NULL) {
        return;
    }

    ncm_song_destroy_unchecked(song);
    return;
}

void
ncm_song_move(NcmSong *dest, NcmSong *source) {
    if (dest == NULL) {
        return;
    }
    if (source == NULL) {
        ncm_song_destroy_unchecked(dest);
        return;
    }

    ncm_song_destroy_unchecked(dest);
    *dest = *source;
    *source = (NcmSong){0};
    return;
}

int32
ncm_song_copy(NcmSong *dest, NcmSong *source) {
    NcmSong replacement = {0};

    if (dest == NULL) {
        return -EINVAL;
    }
    if (source == NULL) {
        return -EINVAL;
    }

    if (source->uri != NULL) {
        ncm_song_set_uri_unchecked(&replacement, source->uri, source->uri_len);
    }

    for (int32 i = 0; i < source->tags_len; i += 1) {
        NcmSongTag *tag = &source->tags[i];

        ncm_song_add_tag_unchecked(&replacement, tag->type,
                                   tag->value, tag->value_len);
    }

    replacement.duration = source->duration;
    replacement.position = source->position;
    replacement.id = source->id;
    replacement.priority = source->priority;
    replacement.last_modified = source->last_modified;

    ncm_song_destroy_unchecked(dest);
    *dest = replacement;
    return 0;
}

int32
ncm_song_from_mpd_song(NcmSong *dest, struct mpd_song *source) {
    return ncm_song_from_mpd_song_copy(dest, source);
}

int32
ncm_song_from_mpd_song_copy(NcmSong *dest, struct mpd_song *source) {
    NcmSong replacement = {0};
    enum mpd_tag_type tags[] = {
        MPD_TAG_ARTIST,
        MPD_TAG_ALBUM,
        MPD_TAG_ALBUM_ARTIST,
        MPD_TAG_TITLE,
        MPD_TAG_TRACK,
        MPD_TAG_NAME,
        MPD_TAG_GENRE,
        MPD_TAG_DATE,
        MPD_TAG_COMPOSER,
        MPD_TAG_PERFORMER,
        MPD_TAG_COMMENT,
        MPD_TAG_DISC,
    };
    char *uri;
    int32 tags_len = (int32)(SIZEOF(tags)/SIZEOF(tags[0]));

    if (dest == NULL) {
        return -EINVAL;
    }
    if (source == NULL) {
        return -EINVAL;
    }

    if ((uri = (char *)mpd_song_get_uri(source)) == NULL) {
        return -NCM_ERROR_NOT_FOUND;
    }

    ncm_song_set_uri_unchecked(&replacement, uri, optional_strlen32(uri));

    for (int32 i = 0; i < tags_len; i += 1) {
        for (uint32 j = 0; ; j += 1) {
            char *value;

            value = (char *)mpd_song_get_tag(source, tags[i], j);
            if (value == NULL) {
                break;
            }
            ncm_song_add_tag_unchecked(&replacement, tags[i], value,
                                       strlen32(value));
        }
    }

    replacement.duration = (int32)mpd_song_get_duration(source);
    replacement.position = (int32)mpd_song_get_pos(source);
    replacement.id = (int32)mpd_song_get_id(source);
    replacement.priority = (int32)mpd_song_get_prio(source);
    replacement.last_modified = mpd_song_get_last_modified(source);

    ncm_song_destroy_unchecked(dest);
    *dest = replacement;
    return 0;
}

int32
ncm_song_set_uri(NcmSong *song, char *uri, int32 uri_len) {
    if (song == NULL) {
        return -EINVAL;
    }
    if (uri == NULL) {
        return -EINVAL;
    }
    if (uri_len < 0) {
        return -EINVAL;
    }

    ncm_song_set_uri_unchecked(song, uri, uri_len);
    return 0;
}

int32
ncm_song_add_tag(NcmSong *song, enum mpd_tag_type type,
                 char *value, int32 value_len) {
    if (song == NULL) {
        return -EINVAL;
    }
    if (value == NULL) {
        return -EINVAL;
    }
    if (value_len < 0) {
        return -EINVAL;
    }
    if (type == MPD_TAG_UNKNOWN) {
        return -EINVAL;
    }

    ncm_song_add_tag_unchecked(song, type, value, value_len);
    return 0;
}

void
ncm_song_set_duration(NcmSong *song, int32 duration) {
    if (song) {
        song->duration = duration;
    }
    return;
}

void
ncm_song_set_position(NcmSong *song, int32 position) {
    if (song) {
        song->position = position;
    }
    return;
}

void
ncm_song_set_id(NcmSong *song, int32 id) {
    if (song) {
        song->id = id;
    }
    return;
}

void
ncm_song_set_priority(NcmSong *song, int32 priority) {
    if (song) {
        song->priority = priority;
    }
    return;
}

void
ncm_song_set_mtime(NcmSong *song, time_t last_modified) {
    if (song) {
        song->last_modified = last_modified;
    }
    return;
}

int32
ncm_song_duration(NcmSong *song) {
    if (song == NULL) {
        return 0;
    }
    return song->duration;
}

int32
ncm_song_position(NcmSong *song) {
    if (song == NULL) {
        return 0;
    }
    return song->position;
}

int32
ncm_song_id(NcmSong *song) {
    if (song == NULL) {
        return 0;
    }
    return song->id;
}

int32
ncm_song_priority(NcmSong *song) {
    if (song == NULL) {
        return 0;
    }
    return song->priority;
}

time_t
ncm_song_mtime(NcmSong *song) {
    if (song == NULL) {
        return 0;
    }
    return song->last_modified;
}

bool
ncm_song_is_empty(NcmSong *song) {
    if (song == NULL) {
        return true;
    }

    return song->uri == NULL;
}

static bool
ncm_song_has_tag_view_unchecked(NcmSong *song, enum mpd_tag_type tag,
                                int32 idx, NcmStringView *view) {
    int32 seen;

    ncm_string_view_clear(view);
    seen = 0;
    for (int32 i = 0; i < song->tags_len; i += 1) {
        if (song->tags[i].type != tag) {
            continue;
        }
        if (seen == idx) {
            ncm_string_view_set(view, song->tags[i].value,
                                song->tags[i].value_len);
            return true;
        }
        seen += 1;
    }

    return false;
}

static bool
ncm_song_has_uri_view_unchecked(NcmSong *song, int32 idx,
                                NcmStringView *view) {
    ncm_string_view_clear(view);
    if (idx != 0) {
        return false;
    }
    if (song->uri == NULL) {
        return false;
    }

    ncm_string_view_set(view, song->uri, song->uri_len);
    return true;
}

static bool
ncm_song_is_stream_unchecked(NcmSong *song) {
    NcmStringView uri;

    if (!ncm_song_has_uri_view_unchecked(song, 0, &uri)) {
        return false;
    }

    return BEGINS_WITH(uri.data, uri.len, "http://")
           || BEGINS_WITH(uri.data, uri.len, "https://");
}

static bool
ncm_song_has_name_view_unchecked(NcmSong *song, int32 idx,
                                 NcmStringView *view) {
    NcmStringView uri;
    int32 basename;

    if (ncm_song_has_tag_view_unchecked(song, MPD_TAG_NAME, idx, view)) {
        return true;
    }
    if (idx != 0) {
        return false;
    }
    if (!ncm_song_has_uri_view_unchecked(song, 0, &uri)) {
        ncm_string_view_clear(view);
        return false;
    }
    if (view == NULL) {
        return true;
    }

    ncm_string_view_clear(view);
    basename = ncm_path_basename_start(uri.data, uri.len);
    ncm_string_view_set(view, uri.data + basename, uri.len - basename);
    return true;
}

static bool
ncm_song_has_directory_view_unchecked(NcmSong *song, int32 idx,
                                      NcmStringView *view) {
    NcmStringView uri;
    int32 basename;

    ncm_string_view_clear(view);
    if (idx != 0) {
        return false;
    }
    if (ncm_song_is_stream_unchecked(song)) {
        return false;
    }
    if (!ncm_song_has_uri_view_unchecked(song, 0, &uri)) {
        return false;
    }
    if (view == NULL) {
        return true;
    }

    ncm_string_view_clear(view);
    basename = ncm_path_basename_start(uri.data, uri.len);
    if (basename == 0) {
        ncm_string_view_set(view, STRLIT("/"));
    } else {
        ncm_string_view_set(view, uri.data, basename - 1);
    }
    return true;
}

static bool
ncm_song_is_from_database_unchecked(NcmSong *song) {
    NcmStringView uri;

    if (!ncm_song_has_uri_view_unchecked(song, 0, &uri)) {
        return false;
    }
    if (uri.len <= 0) {
        return true;
    }
    if (uri.data[0] != '/') {
        return true;
    }

    return ncm_string_find_char(uri.data, uri.len, '/') < 0;
}

bool
ncm_song_has_tag_view(NcmSong *song, enum mpd_tag_type tag, int32 idx,
                      NcmStringView *view) {
    if ((song == NULL) || (idx < 0)) {
        ncm_string_view_clear(view);
        return false;
    }

    return ncm_song_has_tag_view_unchecked(song, tag, idx, view);
}

bool
ncm_song_has_uri_view(NcmSong *song, int32 idx, NcmStringView *view) {
    if (song == NULL) {
        ncm_string_view_clear(view);
        return false;
    }

    return ncm_song_has_uri_view_unchecked(song, idx, view);
}

bool
ncm_song_has_name_view(NcmSong *song, int32 idx, NcmStringView *view) {
    if ((song == NULL) || (idx < 0)) {
        ncm_string_view_clear(view);
        return false;
    }

    return ncm_song_has_name_view_unchecked(song, idx, view);
}

bool
ncm_song_has_directory_view(NcmSong *song, int32 idx, NcmStringView *view) {
    if (song == NULL) {
        ncm_string_view_clear(view);
        return false;
    }

    return ncm_song_has_directory_view_unchecked(song, idx, view);
}

bool
ncm_song_is_from_database(NcmSong *song) {
    if (song == NULL) {
        return false;
    }

    return ncm_song_is_from_database_unchecked(song);
}

bool
ncm_song_is_stream(NcmSong *song) {
    if (song == NULL) {
        return false;
    }

    return ncm_song_is_stream_unchecked(song);
}

static int32
ncm_song_numeric_tag_len_unchecked(char *tag, int32 tag_len) {
    if (ncm_song_needs_numeric_zero(tag, tag_len)) {
        return tag_len + 1;
    }

    return tag_len;
}

int32
ncm_song_numeric_tag_len(char *tag, int32 tag_len) {
    if ((tag == NULL) || (tag_len < 0)) {
        return 0;
    }

    return ncm_song_numeric_tag_len_unchecked(tag, tag_len);
}

static int32
ncm_song_format_numeric_tag_unchecked(char *buffer, int32 buffer_cap,
                                      char *tag, int32 tag_len) {
    return ncm_song_format_numeric_tag_prefix(buffer, buffer_cap,
                                              tag, tag_len, tag_len);
}

int32
ncm_song_format_numeric_tag(char *buffer, int32 buffer_cap,
                            char *tag, int32 tag_len) {
    if ((tag == NULL) || (tag_len < 0)) {
        if ((buffer != NULL) && (buffer_cap > 0)) {
            buffer[0] = '\0';
        }
        return 0;
    }
    if ((buffer == NULL) || (buffer_cap <= 0)) {
        return ncm_song_numeric_tag_len_unchecked(tag, tag_len);
    }

    return ncm_song_format_numeric_tag_unchecked(buffer, buffer_cap,
                                                 tag, tag_len);
}

static int32
ncm_song_show_time_unchecked(int32 length, char *buffer, int32 buffer_cap) {
    int32 hours;
    int32 minutes;
    int32 seconds;

    hours = length/3600;
    length -= hours*3600;
    minutes = length/60;
    length -= minutes*60;
    seconds = length;

    if (hours > 0) {
        return snprintf2(buffer, buffer_cap,
                         "%d:%02d:%02d", hours, minutes, seconds);
    }

    return snprintf2(buffer, buffer_cap, "%d:%02d", minutes, seconds);
}

int32
ncm_song_show_time(int32 length, char *buffer, int32 buffer_cap) {
    if ((buffer == NULL) || (buffer_cap <= 0)) {
        return 0;
    }

    return ncm_song_show_time_unchecked(length, buffer, buffer_cap);
}

static StrBuilder
ncm_song_getter_buffer_unchecked(NcmSong *song, enum NcmSongGetter getter,
                                 int32 idx) {
    StrBuilder buffer = {0};
    NcmStringView view;
    char number_buffer[32];
    int32 copy_len;
    int32 len;
    int32 slash;
    enum mpd_tag_type tag;

    switch (getter) {
    case NCM_SONG_GETTER_LENGTH:
        if (idx > 0) {
            return buffer;
        }
        if (song->duration > 0) {
            len = ncm_song_show_time_unchecked(song->duration, number_buffer,
                                               LENGTH(number_buffer));
            SB_APPEND(&buffer, number_buffer, len);
        } else {
            SB_APPEND(&buffer, "-:--");
        }
        return buffer;
    case NCM_SONG_GETTER_DIRECTORY:
        if (ncm_song_has_directory_view_unchecked(song, idx, &view)) {
            SB_APPEND(&buffer, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_NAME:
        if (ncm_song_has_name_view_unchecked(song, idx, &view)) {
            SB_APPEND(&buffer, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_URI:
        if (ncm_song_has_uri_view_unchecked(song, idx, &view)) {
            SB_APPEND(&buffer, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_TRACK:
        if (ncm_song_has_tag_view_unchecked(song, MPD_TAG_TRACK, idx, &view)) {
            len = ncm_song_numeric_tag_len_unchecked(view.data, view.len);
            sb_reserve(&buffer, len);
            buffer.len = ncm_song_format_numeric_tag_unchecked(
                buffer.data, buffer.cap, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_TRACK_NUMBER:
        if (ncm_song_has_tag_view_unchecked(song, MPD_TAG_TRACK, idx, &view)) {
            slash = ncm_string_find_char(view.data, view.len, '/');
            if (ncm_song_needs_numeric_zero(view.data, view.len)) {
                len = 1;
            } else {
                len = 0;
            }
            if (slash >= 0) {
                copy_len = slash;
            } else {
                copy_len = view.len;
            }
            len += copy_len;
            sb_reserve(&buffer, len);
            buffer.len = ncm_song_format_numeric_tag_prefix(
                buffer.data, buffer.cap, view.data, view.len, copy_len);
        }
        return buffer;
    case NCM_SONG_GETTER_DISC:
        if (ncm_song_has_tag_view_unchecked(song, MPD_TAG_DISC, idx, &view)) {
            len = ncm_song_numeric_tag_len_unchecked(view.data, view.len);
            sb_reserve(&buffer, len);
            buffer.len = ncm_song_format_numeric_tag_unchecked(
                buffer.data, buffer.cap, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_PRIORITY:
        if (idx > 0) {
            return buffer;
        }
        sb_printf(&buffer, "%d", song->priority);
        return buffer;
    case NCM_SONG_GETTER_ARTIST:
    case NCM_SONG_GETTER_ALBUM_ARTIST:
    case NCM_SONG_GETTER_TITLE:
    case NCM_SONG_GETTER_ALBUM:
    case NCM_SONG_GETTER_DATE:
    case NCM_SONG_GETTER_GENRE:
    case NCM_SONG_GETTER_COMPOSER:
    case NCM_SONG_GETTER_PERFORMER:
    case NCM_SONG_GETTER_COMMENT:
        tag = ncm_song_getter_to_tag_type(getter);
        if (ncm_song_has_tag_view_unchecked(song, tag, idx, &view)) {
            SB_APPEND(&buffer, view.data, view.len);
        }
        return buffer;
    case NCM_SONG_GETTER_NONE:
    case NCM_SONG_GETTER_COUNT:
    default:
        return buffer;
    }
}

StrBuilder
ncm_song_getter_buffer(NcmSong *song, enum NcmSongGetter getter, int32 idx) {
    StrBuilder buffer = {0};

    if ((song == NULL) || (idx < 0)) {
        return buffer;
    }

    return ncm_song_getter_buffer_unchecked(song, getter, idx);
}

StrBuilder
ncm_song_tags_buffer(NcmSong *song, enum NcmSongGetter getter,
                     char *separator, int32 separator_len,
                     bool show_duplicates) {
    StrBuilder result = {0};

    if (song == NULL) {
        return result;
    }
    if (getter == NCM_SONG_GETTER_NONE) {
        return result;
    }
    if ((separator == NULL) || (separator_len < 0)) {
        separator = "";
        separator_len = 0;
    }

    for (int32 i = 0; ; i += 1) {
        StrBuilder tag = ncm_song_getter_buffer_unchecked(song, getter, i);
        bool already_present;

        if (tag.len <= 0) {
            sb_free(&tag);
            break;
        }

        already_present = false;
        if (!show_duplicates) {
            for (int32 j = 0; j < i; j += 1) {
                StrBuilder previous
                    = ncm_song_getter_buffer_unchecked(song, getter, j);

                if (STREQUAL(previous.data, previous.len, tag.data, tag.len)) {
                    already_present = true;
                }
                sb_free(&previous);
                if (already_present) {
                    break;
                }
            }
        }

        if (!already_present) {
            if (result.len > 0) {
                SB_APPEND(&result, separator, separator_len);
            }
            SB_APPEND(&result, tag.data, tag.len);
        }
        sb_free(&tag);
    }

    return result;
}

bool
ncm_song_is_equal(NcmSong *a, NcmSong *b) {
    NcmStringView a_uri;
    NcmStringView b_uri;

    if ((a == NULL) || (b == NULL)) {
        return a == b;
    }
    if (!ncm_song_has_uri_view_unchecked(a, 0, &a_uri)) {
        return !ncm_song_has_uri_view_unchecked(b, 0, &b_uri);
    }
    if (!ncm_song_has_uri_view_unchecked(b, 0, &b_uri)) {
        return false;
    }

    return STREQUAL(a_uri.data, a_uri.len, b_uri.data, b_uri.len);
}

#endif /* NCM_SONG_C */

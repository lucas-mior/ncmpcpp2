...
ncm_sample_buffer_destroy(NcmSampleBuffer *buffer) {
    if (buffer->data) {
        ncm_free(buffer->data, buffer->cap*SIZEOF(*buffer->data));
    }
...
ncm_sample_buffer_copy(NcmSampleBuffer *dest, NcmSampleBuffer *source) {
    ...
    dest->len = source->len;
    if (source->len > 0) {
        ncm_memcpy(dest->data, source->data,
...
ncm_sample_buffer_put(NcmSampleBuffer *buffer,
    ...
    if (samples_len > free_samples) {
        ...
        if (kept > 0) {
            ncm_memmove(buffer->data, buffer->data + to_remove,
                        kept*SIZEOF(*buffer->data));
        }
        buffer->len -= to_remove;
    }

    ncm_memcpy(buffer->data + buffer->len, samples,
...
ncm_sample_buffer_get(NcmSampleBuffer *buffer,
    ...
    if (result >= dest_len) {
        ...
        if (dest_len > 0) {
            ncm_memcpy(dest, buffer->data + samples_lost,
                       dest_len*SIZEOF(*dest));
        }
    } else {
        dest_move_len = dest_len - result;
        if (dest_move_len > 0) {
            ncm_memmove(dest, dest + result, dest_move_len*SIZEOF(*dest));
        }
        if (result > 0) {
            ncm_memcpy(dest + dest_move_len, buffer->data,
                       result*SIZEOF(*dest));
        }
    }
    ...
    if (remove_len > 0) {
        ncm_memmove(buffer->data, buffer->data + result,
...
ncm_sample_buffer_resize(NcmSampleBuffer *buffer, int32 cap) {
    ...
    buffer->data = ncm_realloc_array(buffer->data, buffer->cap, cap,
                                     SIZEOF(*buffer->data));
    buffer->cap = cap;

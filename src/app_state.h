#if !defined(APP_STATE_H)
#define APP_STATE_H

#include "cbase.h"
#include "ncmpcpp2.h"

#include "screens/nc_screens.h"

NcScreen *app_state_get_screen(void);
NcScreen *app_state_get_previous_screen(void);
NcScreen *app_state_get_locked_screen(void);
bool app_state_last_switch_has_changed_screen(void);
int32 app_state_register_screen(NcScreen *);
int32 app_state_unregister_screen(NcScreen *);
NcScreen *app_state_find_screen_type(enum NcScreenType);
int32 app_state_switch_to_screen(NcScreen *);
int32 app_state_lock_current_screen(void);
void app_state_unlock_screen(void);
bool app_state_can_show_locked_screen(void);
int32 app_state_show_locked_screen(void);
bool app_state_can_show_inactive_screen(void);
int32 app_state_show_inactive_screen(void);
bool app_state_is_screen_registered(NcScreen *);
bool app_state_is_screen_visible(NcScreen *);
bool app_state_is_current_screen(NcScreen *);
NcScreenResizeParams app_state_screen_resize_params(NcScreen *, bool);
void app_state_request_current_screen_resize(void);
void app_state_request_current_screen_update(void);
void app_state_each_visible_screen(NcScreenEachCallback, void *);
void app_state_update_visible_screens(void);
void app_state_resize_current_screen(void);
void app_state_resize_visible_screens(void);

#endif /* APP_STATE_H */

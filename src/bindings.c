...
binding_action_destroy(BindingAction *action) {
    ...
    case BINDING_ACTION_PUSH_CHARACTERS:
        free2(action->keys.data,
              action->keys.len*SIZEOF(*action->keys.data));
        break;
    case BINDING_ACTION_RUN_EXTERNAL_COMMAND:
    case BINDING_ACTION_RUN_EXTERNAL_CONSOLE_COMMAND:
        free2(action->argument.data, action->argument.cap);
        break;
...
binding_action_copy(BindingAction *dest, BindingAction *source) {
    ...
    case BINDING_ACTION_REQUIRE_SCREEN:
        ...
        break;
    case BINDING_ACTION_PUSH_CHARACTERS:
        dest->keys.len = source->keys.len;
        if (source->keys.len > 0) {
            int32 bytes;

            bytes = source->keys.len*SIZEOF(*dest->keys.data);
            dest->keys.data = malloc2(bytes);
            memcpy64(dest->keys.data, source->keys.data, bytes);
        } else {
            dest->keys.data = NULL;
        }
        break;
    case BINDING_ACTION_RUN_EXTERNAL_COMMAND:
    case BINDING_ACTION_RUN_EXTERNAL_CONSOLE_COMMAND:
        dest->argument.len = source->argument.len;
        if (source->argument.len > 0) {
            dest->argument.data = ncm_string_copy(source->argument.data,
                                                  source->argument.len,
                                                  &dest->argument.cap);
        } else {
            dest->argument.data = NULL;
            dest->argument.cap = 0;
        }
        break;
    default:
...
void
binding_destroy(Binding *binding) {
...
void
binding_append_action(Binding *binding, BindingAction *action) {
    BindingAction copy;
...
void
binding_copy(Binding *dest, Binding *source) {
    *dest = (Binding){0};
...
bool
binding_can_execute_default(Binding *binding) {
...
int32
binding_execute_default(Binding *binding) {
    BindingRuntime *runtime;
    int32 status;
    ...
    runtime = binding_default_runtime();
    for (int32 i = 0; i < binding->actions_len; i += 1) {
        BindingAction *action = &binding->actions[i];
        ...
        case BINDING_ACTION_NORMAL:
            ...
            break;
        case BINDING_ACTION_PUSH_CHARACTERS:
            for (int32 j = 0; j < action->keys.len; j += 1) {
                runtime->push_key(action->keys.data[j], runtime->user);
            }
            status = 0;
            break;
        case BINDING_ACTION_REQUIRE_SCREEN:
        case BINDING_ACTION_REQUIRE_RUNNABLE:
            ...
            break;
        case BINDING_ACTION_RUN_EXTERNAL_COMMAND: {
            BindingActionArgument *arg = &action->argument;
...
void
bindings_config_destroy(BindingsConfiguration *bindings) {
...
static void
binding_append_normal(Binding *binding, enum ActionType type) {
...
static int32
bindings_key_lower_bound(BindingsConfiguration *bindings, NcKey key) {
    int32 first = 0;
...
static int32
bindings_key_index(BindingsConfiguration *bindings, NcKey key) {
    int32 at;
...
static bool
bindings_key_is_unbound(BindingsConfiguration *bindings, NcKey key) {
    int32 at;
...
#define NCM_DEFAULT_BINDINGS(XX_SEQ, XX_GROUP)                                 \
  ...
  XX_GROUP("n", ACTION_MOVE_SORT_ORDER_DOWN, ACTION_MOVE_SELECTED_ITEMS_DOWN)  \
...
binding_parse_directive(BindingAction *action, BindingDirective *directive,
    ...
    case BINDING_DIRECTIVE_PUSH_CHARACTER: {
        ...
        action->keys.len = 1;
        action->keys.data = malloc2(SIZEOF(*action->keys.data));
        action->keys.data[0] = action_key;
        return 0;
    }
    case BINDING_DIRECTIVE_PUSH_CHARACTERS:
        ...
        action->kind = BINDING_ACTION_PUSH_CHARACTERS;
        action->keys.len = argument.len;
        action->keys.data = malloc2(action->keys.len
                                    *SIZEOF(*action->keys.data));
        for (int32 i = 0; i < argument.len; i += 1) {
            action->keys.data[i] = (NcKey)(uint8)argument.data[i];
        }
        return 0;
    ...
    case BINDING_DIRECTIVE_EXTERNAL_CONSOLE_COMMAND:
        ...
        action->argument.data = ncm_string_copy(argument.data, argument.len,
                                                &action->argument.cap);
        action->argument.len = argument.len;
        return 0;
    default:
        UNREACHABLE();
    }
...
binding_parse_action_line(BindingAction *action, char *line, int32 line_len,
    ...
    if (name_len == line_len) {
        if (ncm_action_type_parse(line, name_len, &action->type) < 0) {
...
void
bindings_config_generate_defaults(BindingsConfiguration *bindings) {
    Binding binding = {0};
